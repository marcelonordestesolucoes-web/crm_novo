...
export default function FlowNodeCard({ id, data, selected }) {
  ...
  const isCondition = node.type.startsWith('condition_') || node.type === 'action_validation';
  ...
  const renderMessageInput = ({ placeholder = 'Digite sua mensagem', withAttach = false } = {}) => (
    <div className="border-t border-b border-slate-200/80 bg-white/90 px-3 py-1.5">
      {!textActivated ? (
        <div className="flex items-center gap-1.5 flex-nowrap">
          <div className="flex items-center gap-0.5 shrink-0">
            <button type="button" onClick={(event) => event.stopPropagation()} className="nodrag nopan flex h-7 w-7 items-center justify-center rounded-md text-slate-400 hover:bg-slate-100">
          ...
          <div className="relative flex-1 min-w-0">
            <input
              ...
              className="nodrag nopan h-8 w-full rounded-lg border border-slate-100 bg-slate-50/50 px-2.5 text-[12px] font-semibold text-slate-700 outline-none focus:border-slate-300 focus:bg-white transition-all"
            />
          </div>
          <button
            type="button"
            ...
            className={cn(
              'nodrag nopan flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border transition-all',
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="nodrag nopan h-8 w-14 shrink-0 rounded-lg border border-slate-200 bg-white text-[10px] font-black uppercase text-slate-400 hover:bg-slate-50 transition-all"
          >
            Editar
          </button>
        </div>
      ...
      <div className="space-y-1.5 border-b border-slate-200/80 px-5 py-3">
        ...
        {options.map((option, index) => (
          <div key={`${id}-option-${index}`} className="group/option flex items-center gap-3 relative">
            ...
            <NodeHandle id={`option_${index}`} className="!right-[-28px] !h-3.5 !w-3.5" />
          </div>
        ))}
      </div>
  ...
  const renderSimpleConfig = () => {
    ...
    if (isCondition) {
      if (node.type === 'condition_validation' || node.type === 'action_validation' || meta.title === 'Validar Formato') {
        return (
